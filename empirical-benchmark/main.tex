...
is \natLarge{} B/req against \texttt{fasthttp}'s \fastLarge. Throughput was
measured on a machine at a load average of \loadLo--\loadHi{} across ten cores,
where a single arm's sequential rate varies by a factor of \spreadFactor{} between
repetitions of the identical loop. Under concurrency both ZAP arms beat
\texttt{net/http} by $\zapVsStdLo$--$\zapVsStdHi\times$ in every paired
repetition; against \texttt{fasthttp} the ratio brackets $1.0$ in both directions,
and this machine cannot separate them. Every number here is a line in
...
\paragraph{Workloads.} \nWorkloads{} bodies: tiny (16\,B, 4 headers), small
...
\bottomrule
\end{tabular}
\caption{Requests per second, \nWorkers{} concurrent workers, \concReqs{} requests, median of
\nReps{} repetitions. Taken at a load average between \loadLo{} and \loadHi{} on
ten cores; the
absolute rates are not a statement about the hardware.}
\end{table}
...
cannot be separated on this machine: whatever ZAP-HTTP saves in allocation
does not show up as throughput against a client that was already allocation-free.
Reporting a median here would manufacture a winner out of three numbers that
disagree about who won.

We make no claim at all from the sequential rates either. Across three repetitions
of the identical \memReqs-request loop in the stored run, the \spreadArm{} arm on the
\spreadWorkload{} body measured \spreadValues{} \texttt{req/s}. The previous revision's
$1.85$--$3.68\times$ throughput claim, and its bandwidth table (1\,233\,MB/s
native against HTTP's 519), are therefore neither confirmed nor refuted here; they
...
per-request floor and serves $\zapVsStdLo$--$\zapVsStdHi\times$ more requests in
every paired repetition under contention. That floor is the number a stdlib service would
actually see, and it is worth more than the four-digit ratio the floor produces
when divided by a rounding artifact. Against \texttt{fasthttp}, which already has
no such floor, the throughput margin is not resolvable on this machine and we do
not claim one.
